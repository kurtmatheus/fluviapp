...
class ViagemDadosViagemMapperTest {
    ...
    @Test
    fun `resolve empresa e embarcacao pelo id (Viagem nao guarda mais os nomes)`() = runTest {
        val viagem = Viagem(
            id = "v1",
            codigo = "COD",
            origem = "Porto Norte",
        ...
        assertEquals("F/B ATUAL", card.embarcacao)
        assertEquals("60", card.capacidadeVeiculos) // capacidades vêm do embarcacao resolvido
        assertEquals("Porto Norte", card.origem)
        assertEquals("Ilha Central", card.destino)
    }

    @Test
    fun `id nao resolvido deixa empresa e embarcacao vazios`() = runTest {
        val viagem = Viagem(
            id = "v1",
            codigo = "COD",
            origem = "Porto Norte",
        ...
        assertEquals("0", card.capacidadeVeiculos) // sem embarcacao resolvido
    }
}
