...
/**
 * v16 → v17: remove `Passagem.desconto`. O desconto deixou de ser digitado e persistido (ADR-0013: é
 * DERIVADO da tarifaBase — resíduo abaixo da tarifa devida); não há bilhete legado que dependa do
 * fallback (decisão do analista). SQLite não garante DROP COLUMN no minSdk 26, então recria a tabela
 * (padrão da 4→5/6→7/8→9), copiando as demais colunas. Passagem no Room é cache do Firestore (verdade),
 * então recriar é seguro (re-sincroniza).
 */
    ...
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS `Passagem_novo` (" +
                ...
                "`agencia` TEXT NOT NULL, `agente` TEXT NOT NULL, `valorPago` REAL, `valorPix` REAL, " +
        ...
        db.execSQL(
            "INSERT INTO `Passagem_novo` (" +
                ...
                "`origem`, `destino`, `dataViagem`, `horaViagem`, `agencia`, `agente`, `valorPago`, " +
                ...
                "`valorPago`, `valorPix`, `valorDinheiro`, `valorDebito`, `valorCredito`, `tarifaBase`, " +
