...
| [capacidades-do-adm.md](capacidades-do-adm.md) | **base** · §4 respondida em 2026-09-08 | *Por que dar capacidade ao `ADM` é caro, e o que as métricas podem medir.* Nasce de uma pergunta do analista, e responde com números: a política tem **26 funções, 15 delas fundindo `ADM` e `GESTOR`** (`ehPapelPlataforma`) e **3 distinguindo o `ADM`** — nas regras, 21 contra 7. Daí a resposta curta: **não existe lugar onde o `ADM` seja o sujeito da frase**, então toda capacidade nova começa por separá-lo do `GESTOR` nos dois eixos ao mesmo tempo. Mede mais seis causas, e **separa o que é acoplamento do que é preço de invariante** — o anti-escalonamento de `users/{uid}` é parede deliberada, não dívida: a gestão de acesso custou **13 arquivos e 958 linhas** para duas escritas de dois campos, e o teto de **1000 expressões** da linguagem de regras é recurso compartilhado por todas as escritas. Para as métricas, derruba a premissa da própria issue — **o evento da D4 não alimenta tela nenhuma** (o SDK do Analytics é só de escrita; os eventos vivem no console/BigQuery) — e mostra que a régua sobrevive à troca de fonte: o Firestore **conta sem devolver documento** (`count()`), o que é a D4 aplicada à leitura. E acha que `CriterioPassagem` **já sabe recortar** por escopo, período, status e categoria: falta a pergunta *"quantos"* ao lado de *"quais"* |
