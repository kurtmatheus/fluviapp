...
**Emendado em 2026-08-17**, e desta vez por **dois defeitos medidos no app distribuído**, não por documento
novo. Uma viagem e uma rota inativadas pelo painel não saíram do card da tela inicial, e o rastro cruzou três
ADRs: o [0009](0009-sincronizacao-reativa-firestore-room.md) (o pipeline estava reativo; o **consumidor** não
assinava), o [0017](0017-eixo-de-storage-firestore-only.md) (trocar o datasource não migra quem o lê) e o §7.1
do [0016](0016-dominio-da-plataforma.md) (o recorte tratava *rota inativada* como *rota existente*). Ver a
seção *[As duas dessincronizações](#as-duas-dessincronizações-2026-08-17)*. Na mesma leva o
[0030](0030-o-bilhete-digital.md) ganhou emenda de **forma**: o bilhete digital deixou de herdar o desenho da
bobina de 80 mm.

**Emendado em 2026-08-18** pela **menor unidade de emenda que este índice registra: uma troca de
biblioteca**. O leitor de QR do embarque saiu de **CameraX + ML Kit** para o **`ScanContract` do
zxing-android-embedded**, com o formato travado em QR. Nenhuma decisão de domínio se move — muda **quem
hospeda a câmera** —, e por isso ela toca exatamente duas linhas: a *Lib do scanner* do
[0012](0012-ciclo-de-vida-passagem-e-embarque-qr.md) e a promessa de que *"o scanner não muda"* do
[0026](0026-orquestracao-e-apresentacao-da-passagem.md). Fica registrada porque **paga uma dívida que estava
anotada em outro documento**: o `.so` do CameraX 1.3.4 era o único bloqueio do app para o requisito de
**16 KB** do `targetSdk 35` ([`docs/design/camada-de-apresentacao.md`](../design/camada-de-apresentacao.md)),
...
código sustentaram a virada, e duas delas são de peças que **ninguém lia**: `ehPesado` sem leitor e
`tarifaMotoBase` sem chamador. O ADR toca duas linhas do quadro — a sua e a do [0016](0016-dominio-da-plataforma.md),
cujo §8 muda de forma e de conteúdo.
...
| [0009](0009-sincronizacao-reativa-firestore-room.md) | Pipeline reativo único | **vigente · parcial** | o **destino** muda: o DAO deixa de ser a fonte reativa e vira `StateFlow` — ADR-0017 D1. Ciclo de vida, porta `FonteSnapshots` e telemetria seguem. **2026-08-17 — o limite do "único":** o pipeline estava certo e a tela não. O Início lia `obterTodas()` uma vez no `init` e ficava com a fotografia enquanto o ViewModel vivesse; cada tela recarregava a si mesma depois da **própria** escrita, e nenhuma sabia da escrita das outras — dois mecanismos que o ADR-0009 já apontava como desconectados, agora com sintoma no tester. **Reativo no repositório não é reativo na tela**: o consumidor tem de assinar |
| [0010](0010-autorizacao-por-cargo.md) | Política única de autorização | **vigente · parcial** | os cargos foram **renomeados** (ADR-0015) e a política ganhou a **atuação** como terceira coordenada (ADR-0016, 8ª rodada). **Estendido em 2026-09-07** ([0032](0032-o-acesso-politica-sessao-e-ciclo-de-vida.md) D1): o escopo declarado aqui — *"segurança por UI"* — deixa de bastar, e a política passa a ser consultada **antes do gesto**, não só antes de desenhar o menu. A fronteira que vale continua sendo o servidor; o que o cliente ganha é **falhar antes** e **falhar explicando** |
...
| [0012](0012-ciclo-de-vida-passagem-e-embarque-qr.md) | FSM da passagem + embarque por QR | **vigente · parcial** | **cancelar deixa de ser *delete* físico** e vira estado — ADR-0018 D17. O carimbo de embarque vira sub-objeto — D14 — e **perde o nome** de quem embarcou: fica `embarcadaPorId` + `embarcadaEm` (ADR-0023 D8). A FSM e o QR como ponteiro seguem inteiros, e são a parte da F9 que **não se refaz**. **2026-08-18 — a *lib do scanner* caiu** (decisão do analista): *CameraX + ML Kit* dá lugar ao **`ScanContract` do zxing-android-embedded**, `desiredBarcodeFormats = QR_CODE`. É troca de **implementação**, não de decisão — a câmera deixa de ser hospedada na tela (preview vinculado ao ciclo de vida do Composable, análise quadro a quadro, `ImageProxy` fechado à mão) e a leitura passa a ser **um resultado que a tela pede**: 106 linhas fora, 55 dentro, cinco dependências a menos. Efeitos que valem além do arquivo: **gerar e ler o QR passam a viver na mesma biblioteca** (o ZXing já gerava), o modelo do ML Kit sai, e o `.so` do CameraX sai do APK. O que **não** se move: a FSM, o QR como ponteiro, o `onQrLido` e a permissão pedida **na tela** antes de lançar — a biblioteca também a pede, mas quem nega dentro dela recebe só um resultado vazio, e a tela ficaria muda |
...
| [0021](0021-usuarios-da-plataforma-adm-only.md) | Usuários da plataforma (`ADM`-only) | **direção · FORA DO MVP** (D0) | **não implementar**: o cadastro no console vira **princípio** — a administração da plataforma vive fora do app, somando P2.2c + anti-escalonamento + fim do seed + ADR-0016 §10. D1–D4 valem como desenho de quando a seção nascer: primeira divergência entre `ADM` e `GESTOR`, só leitura, e `allow read` de `users` restrito. **O D2 caiu em 2026-09-07** ([0032](0032-o-acesso-politica-sessao-e-ciclo-de-vida.md) D6): a seção deixa de ser somente-leitura e o `ADM` passa a **gerir** gestores e operadores — editar, desativar, reativar, expirar. **O D0 fica**, e sai reforçado: não há convite de `ADM`. O que o mesmo 0032 registra é que a leitura literal do D0 (*"não existe caminho, dentro do app, para fabricar quem administra"*) **já era falsa quando foi escrita** — quatro dias depois, a F6.6 fez o papel vir do convite, e nenhum ADR revisou. **Implementado em 2026-09-08**: das três escritas que o D2 recusava, **desligar** aconteceu como ele mesmo pedia (campo `ativo`, não `delete`, com reativar ao lado), **convidar** já tinha acontecido na F6.6, e **promover/rebaixar** continua não existindo — o `papel` está fora da lista fechada de chaves, inclusive para o `ADM`. O **D3** (`allow read` de `users` restrito ao `ADM` e ao próprio dono) segue **não implementado**, e agora tem um leitor a mais a considerar: é pela leitura ampla que o app pode explicar a alguém por que o acesso dele não vale |
...
| [0031](0031-classe-de-veiculo-natureza-e-casco-por-exclusao.md) | A classe de veículo — enum, natureza e casco por exclusão | **vigente · IMPLEMENTADA** (2026-09-03) | **escrito e executado no mesmo dia**, em quatro commits (`e749fb6`, `f901653`, `3807657`, `3f7c63a`) — ver *[O que a execução do 0031 acrescentou](#o-que-a-execução-do-0031-acrescentou)*. Nasce da issue #4 (*"o supervisor cadastrar classes"*) e **confirma por outro caminho** a decisão de 2026-08-01 (*a classe não é catálogo editável*): nem entidade, nem catálogo — **o enum é o registro**, porque ele é a chave de uma série histórica e série não convive com identidade que se renomeia. Aplica o mecanismo do `Cargo` **sem a segunda metade dele**: bastava que poder e registro deixassem de morar na mesma palavra. A propriedade que fica é a **natureza** (automotor · cilindrada · máquina autopropelida · rebocado), e ela existe **pela capacidade analítica** — *"data-intensive: guardar todas as informações possíveis"* —, não pela arrumação do código; com ela `exigeCilindrada` passou a **derivar** — e **voltou a ser declarada em 2026-09-05**, ver abaixo. A lista fecha em **dezessete** (onze novas), e a `Carreta Cavalinho` é **tratora, motorizada** — o caso que prova o D2, porque o nome não deixava derivar a natureza. **Revisa o [0016](0016-dominio-da-plataforma.md) §8 em forma e conteúdo:** o casco admite **por exclusão** (Balsa **todas** · Navio **apenas carro e moto** · Lancha **nenhuma**), de modo que classe nova **nasce vendável** sem decisão humana — o oposto do "Catamarã inerte". Caem o **porte** (perdeu o consumidor que o justificava) e o `ehPesado` (nunca teve leitor); `tarifaMotoBase` fica sem razão. A emissão ganha **passo + subpasso** (natureza → classe), que também separa `Carreta`, `Carretilha` e `Carreta Cavalinho` de um seletor único. A classe é **capacidade compartilhada sem concessão** — diferente de Rota e Viagem, toda agência dispõe de todas. **Supera o §6.4 do estudo**, que concluía por dois eixos |
| [0032](0032-o-acesso-politica-sessao-e-ciclo-de-vida.md) | O acesso — política, sessão e ciclo de vida | **vigente · em implementação** (§Q respondida; **D1–D6 no código, D6 sem as métricas**) | **O que já está de pé (2026-09-08):** D1 (política antes do gesto + guarda de navegação), D2 (sessão como fluxo), D3 (gate inteiro — os 57 casos escondidos passaram de primeira), D4 (evento agregado + embarque emitindo + 23 `Log.e` na telemetria), D5 (dois perfis, troca no menu, `vinculo` singular) e D6 (convite sem `ADM`, acesso com estado, seção Usuários escrevendo). **Falta** só a tela de métricas de gestão de usuários, que depende de decidir *quais perguntas ela responde*. Duas coisas que a execução acrescentou às decisões: **o dono não mexe no próprio acesso** e **ninguém desativa a si mesmo** — sem a primeira, o desativado se religaria pelo ramo do dono, que não pergunta o papel. E a suíte de regras achou o **teto de 1000 expressões** da linguagem quando o acesso passou a ter estado: a linguagem não memoriza resultado de função, então `souSupervisorDe()` morreu (os três chamadores já perguntavam o cargo antes) e o limite do prazo passou a entrar por parâmetro. · novo, e fecha o eixo que nunca tinha sido medido junto (estudo [`acesso-e-identidade.md`](../design/acesso-e-identidade.md)). Seis decisões. **D1 — a política volta a ser consultada antes do gesto**: 12 das 22 funções de `PermissoesUsuario` não tinham chamador de produção, e `emitir()`/`confirmarEmbarque()` agiam sem perguntar, de modo que **a única fronteira ativa era o servidor**; estende o [0010](0010-autorizacao-por-cargo.md) no ponto em que ele se dizia "segurança por UI" — o cliente não vira autoridade, ganha **falhar antes** e **falhar explicando**. **D2 — a sessão é estado observável**: a porta ganha `Flow` e **dissolve três acoplamentos de uma vez** — o menu larga o DataStore cru sem perder reatividade (a leitura crua não era desleixo: a porta só sabia responder uma vez), a chave `LOGADO` morre (escrita duas vezes, lida por ninguém — resíduo de uma decisão do [0005](0005-autenticacao-sessao-firebase-datastore.md) que a execução superou por um caminho melhor) e a escrita dupla do login some. **D3 — o gate das regras roda inteiro**: `npm test` pulava **57 de 160 casos**, justo os de passagem, cliente e veículo; o recorte era o da revitalização, e **o andaime já se esvaziou** (9 seções de 9). Liga, roda e **analisa** — o vermelho é medição. **D4 — o evento é agregado, o carimbo é a auditoria**: o operador entra como coordenada (agência/papel/cargo), nunca identidade — *o evento conta, o documento prova*; entram o evento de embarque (que não existia) e os 23 `Log.e` que passavam ao largo da telemetria. **D5 — dois perfis por dono** (plataforma + no máximo uma empresa), troca pelo menu, só `ADM`/`GESTOR`: **supera o [0016](0016-dominio-da-plataforma.md) §6 e a F6.4** — a escolha entre duas empresas morre, e ~284 linhas com ela. Não custa regra nova porque **o servidor já concede a união** (papel + cargo no mesmo uid): a troca é **lente, não redução de poder**, o que seria inaceitável para um `OPERADOR` e é aceitável para quem a decisão autoriza. E o invariante *"ADM/GESTOR não emitem"* do [0015](0015-rework-agente-equipe.md) §8.4 **não quebra**: a regra barra por `funcionarioIdDoAutor()` vazio, não por papel — com perfil de empresa ele emite **como o funcionário**, e a passagem nasce com o dono certo. **D6 — o `ADM` gere gestores e operadores** (editar, desativar, reativar, **data de expiração**, métricas): **supera o [0021](0021-usuarios-da-plataforma-adm-only.md) D2** e **preserva o D0** — não há convite de `ADM`, que continua entrando por console. Corrige de passagem o código que oferecia os três papéis no formulário. **As três perguntas foram respondidas no mesmo dia (§Q).** O estado do acesso (`ativo`/`expiraEm`) mora **no próprio `users/{uid}`**, com o ADM restrito a uma lista fechada de chaves e o `papel` fora dela **para todo mundo** — o argumento decisivo é de custo de leitura: `papel()` já faz esse `get()`, então a verificação sai de graça, enquanto guardar fora cobraria um salto a mais em toda operação do app. **Expirar impede a próxima operação e não derruba a sessão em curso** — quem está no meio de um atendimento termina o atendimento. `Funcionario.vinculos` **deixa de ser lista** (vira `vinculo: Vinculo?`, mesmo princípio da cilindrada no 0031). E a troca de perfil se anuncia como **carregamento** e nada mais — *"carregando sessão e informações do perfil"* com indicador circular padrão, porque celebrar a troca sugeriria uma separação que o servidor não faz |
...
**4. A operação corrigiu o domínio dois dias depois, e derrubou a única inferência do ADR** (2026-09-05).
Ver a *Emenda ao D2* no próprio documento: o **jet-ski é rebocado** (moto aquática no nome, carga rebocada
na doca) e o **quadriciclo não exige cilindrada** — só a moto exige. Duas classes da mesma natureza com
exigências diferentes não cabem numa coluna derivada, então `exigeCilindrada` voltou a ser **declarada**,
com um `true` em dezessete linhas. O que o episódio ensina não é sobre veículos: a derivação se apoiava no
jet-ski como prova de que o traço era de família, e o jet-ski era o valor **classificado errado** — corrigido
o fato, o padrão se desfez sozinho. Na mesma leva, a lista de classes do cartão virou **texto de suporte**
(`bodySmall`): ela saía no corpo e no peso do rótulo, e a natureza sumia no meio de sete nomes em negrito.
**823 casos JVM e 152 instrumentados verdes**; as duas correções provadas por mutação — desfeitas, caem
**oito casos** em três arquivos.
...
**Emenda de 2026-08-17 — a rota inativada entra no recorte.** O `EscopoDoPool` derrubava a viagem só quando
a rota **não estava no mapa**, e a porta da Rota não tem `deletar`: o que existe é `Rota.ativo`. A rota tirada
de circulação continuava no mapa, continuava autorizando as viagens dela, e o caso que a regra descrevia
("rota apagada") era **inalcançável** enquanto o caso real passava livre. O argumento estava escrito um nível
acima, no cadastro de viagem, que já só oferece rota ativa *"porque oferecer uma inativada seria criar uma
partida nascida sobre o que já foi encerrado"*: se **ver e vender são a mesma pergunta**, o que não se pode
criar não se pode vender. A assimetria da plataforma segue de propósito e agora tem dois casos — ela vê a
órfã **e** a de rota inativada, porque é ela quem reativa a rota, e nada foi apagado.
...
| `ClasseVeiculo` como **tabela de propriedades** (`exigeCilindrada`, `ehPesado`, `exigeModelo` + pertencimento enumerado) | **registro** de dezessete valores, cada um declarando a **natureza**; `ehPesado` e `exigeModelo` morrem, e `exigeCilindrada` sobra como **exceção única** (só a moto) | ADR-0031 D1/D2 + emenda de 2026-09-05 |
| `TipoEmbarcacao.classesAdmitidas: Set<ClasseVeiculo>` | admissão **por exclusão**: *todas · apenas(conjunto) · nenhuma* | ADR-0031 D4 |
...
| `Funcionario.vinculos: List<Vinculo>` + o derivado `empresaIds` | **`vinculo: Vinculo?`**, um mapa no documento e **sem derivado** — o `empresaIds` existia só porque array não se consulta por dentro | ADR-0032 Q2, implementado em 2026-09-08 |
| `EscolhaDeVinculo` / *"em nome de qual empresa opero"* (a seleção de contexto, F6.4) | **`PerfilAtivo`** / *"qual perfil está ativo"* — mesma forma (preferência revalidada, nunca autoridade), outro eixo. A escolha **entre empresas** não existe mais | ADR-0032 D5, implementado em 2026-09-08 |
| acesso como *existir `users/{uid}`* | acesso com **estado**: `ativo` + `expiraEm` no próprio documento, lidos pela regra em `papel()` e em `funcionarioIdDoAutor()` | ADR-0032 D6/Q1, implementado em 2026-09-08 |
| `model/` (pacote) | `domain/` | rename de 2026-07-31 |
| Room como *datasource* | cache do SDK + `StateFlow` por coleção | ADR-0017 |
| Room, ponto — banco local, `@Entity` no domínio, migração, schema exportado | **não existe** (2026-09-07): o que é local por natureza mora no **DataStore** | ADR-0017 F6 |
| `BilheteDigital.travessia` | **`trajeto`** — no bilhete o valor **leva rótulo**, e campo e rótulo com o mesmo nome é o que deixa ler o DTO e o documento como a mesma coisa. A `ConferenciaDeEmbarque` e o `CabecalhoDaViagem` seguem com `travessia` e **sem** rótulo: ali o valor é o assunto da tela, não um campo dela | ADR-0030, emenda de 2026-08-17 |
| "scanner **CameraX + ML Kit**" (a lib do embarque) | **zxing-android-embedded** — `ScanContract` + `ScanOptions.QR_CODE`: a tela **pede um resultado** em vez de hospedar a câmera, e ler o QR volta para a biblioteca que já o gerava | 2026-08-18 |
...
- **Se o Início ainda quer pull-to-refresh** (F10). Ele saiu com a lista antiga e não voltou; o que ele fazia
  era forçar `Source.SERVER` para reencher o Room, e nem o Room nem aquele repositório existem. Com o Início
  **assinando** a fonte desde 2026-08-17, o gesto perdeu a função técnica — o que sobra é a **expectativa** de
  quem opera, que puxa a lista para ter certeza. Decidir se ele volta como confirmação (sem nada a buscar) ou
  não volta.
...
## As duas dessincronizações (2026-08-17)

Uma viagem e uma rota inativadas pelo painel **continuaram no card da tela inicial**. O relato veio do app
distribuído, e o card só sumia ao reabrir o app — o que já separava as duas causas antes de qualquer leitura
de código:

| O defeito | Onde estava | Por que passou tanto tempo |
|---|---|---|
| O Início ficava com a **fotografia** da primeira leitura | `MainScreenViewModel.carregarInicio()` lia `obterTodas()` no `init`, e o VM vive enquanto a home está na pilha | cada tela recarregava a si mesma depois da **própria** escrita, então quem inativava **via** o efeito na hora. Só quem olhava de outra tela não via |
| **Rota inativada** continuava autorizando as viagens dela | `EscopoDoPool`: `List<Viagem>.noEscopo` descartava a viagem só quando a rota **faltava no mapa** | a porta da Rota não tem `deletar`, então "rota apagada" nunca acontecia — a regra descrevia um caso inalcançável e deixava o real passar |

**O que os dois ensinam junto.** O primeiro é um defeito de **camada**: o dado sempre esteve vivo
(`ColecaoFirestore` mantém listener e `StateFlow`, e o próprio `obterTodas()` **liga** esse listener), e
faltava o consumidor assinar. O segundo é de **domínio**: um estado que existe (`ativo`) sem ninguém que o
leia é igual a não existir. Nenhum dos dois aparece em teste de unidade de camada isolada, e nenhum dos dois é
"bug de UI".

**Onde a correção mora, e por quê.** A montagem do Início virou `fluxoDoInicio`, **fora do ViewModel** — pelo
mesmo motivo que já tirara o `InicioDaTelaMapper` de lá: `MainScreenViewModel` depende de `FirebaseAuth` e de
`DataStore` e não se constrói numa JVM, e é por isso que nunca teve teste. Com a montagem fora, **a
reatividade passou a ter teste** (era justamente ela que não tinha): seis casos que mudam o fake **depois** de
o fluxo já estar coletando e nunca chamam recarga — se passassem com uma recarga explícita, provariam o
contrário do que interessa.

**Método que vale registrar:** as duas correções foram checadas por **mutação**. Com o `takeIf { it.ativo }`
removido, dois casos falham; com ele de volta, verdes. Um teste de regressão que não se prova falhando contra
o defeito original é uma afirmação sobre o código, não uma medida dele. Suíte após as duas: **789 casos JVM**.
