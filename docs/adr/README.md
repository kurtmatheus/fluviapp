...
| [0021](0021-usuarios-da-plataforma-adm-only.md) | Usuários da plataforma (`ADM`-only) | **vigente** · sem código ainda | primeira decisão em que **`ADM` e `GESTOR` divergem** — até aqui `ehPapelPlataforma()` os fundia em toda decisão. Só leitura, e aperta o `allow read` de `users` para `ehAdm()` ou o próprio dono. **Emenda o ADR-0011**: a paridade Kotlin ↔ rules passa a ter dois predicados de plataforma |
